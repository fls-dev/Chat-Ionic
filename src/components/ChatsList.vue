...
<ion-content  class="ion-padding" :fullscreen="true">
  <ion-header collapse="condense">
    <ion-toolbar>
      <ion-title size="small">Chats</ion-title>
    </ion-toolbar>
  </ion-header>

  <ion-item-sliding v-for="chat in chats.chats">
    <ion-item lines="full" class="el-chat" @click="openChat(chat.nameChat)">
      <ion-avatar slot="start">
        <img :src=chat.avatar>
      </ion-avatar>
      <ion-label class="elem-list">
        {{ chat.nameChat }}
        <p>{{ chat.lastMessage.slice(0, 90) }}</p>
      </ion-label>
      <div class="last-data">{{chat.dataLastMessage}}</div>
    </ion-item>

    <ion-item-options side="end">
      <ion-item-option color="danger"> <ion-icon :icon="trashOutline" /></ion-item-option>
      <ion-item-option color="primary"> <ion-icon :icon="createOutline" /></ion-item-option>
    </ion-item-options>
  </ion-item-sliding>
...
import {
  IonContent,
  IonHeader,
  IonLabel, IonPage, IonTitle, IonToolbar, IonAvatar, IonIcon
} from "@ionic/vue";
import {trashOutline, createOutline} from 'ionicons/icons';


export default defineComponent({
  components: {IonContent, IonLabel, IonHeader, IonToolbar, IonTitle,IonIcon, IonPage, IonAvatar},
  name: 'ChatsList',
  setup() {
    return {
      trashOutline,
  ...
  methods:{
    openChat(id){
      this.$router.push ({path: `/tab1/message/${id}`, meta: {id: id}, replace: true})
    }
  }
});
</script>
